  ...
  try {
    ...
    if (count > 0) {
      ...
      if (field.inputType === 'select') {
        ...
        const scopedFieldset = page.locator('fieldset, li, [class*="question"], [class*="field"]', { hasText: field.label }).first();
  ...
  try {
    ...
    if (!company) {
      try {
        ...
        // jobs.smartrecruiters.com/<CompanyName>/<job-id>
...
export class SmartRecruitersAdapter {
  ...
  canHandle(url) {
    try {
      const u = new URL(url);
      return u.hostname.includes('smartrecruiters.com');
    } catch {
      return String(url || '').includes('smartrecruiters.com');
    }
  }
  ...
  async run(params) {
    ...
    // If on a job listing page (no /apply in path), try to navigate to apply form
    try {
      const u = new URL(pageUrl);
      if (!u.pathname.includes('/apply')) {
        // Try clicking an "Apply" button first
        const APPLY_BUTTON_PATTERNS = [/^i want to apply$/i, /^apply now$/i, /^apply$/i, /^start application$/i];
        let clicked = false;
        ...
        // Fallback: append /apply to URL
        if (!clicked) {
          const applyUrl = pageUrl.replace(/\/$/, '') + '/apply';
    ...
    await page.waitForSelector('form', { state: 'visible', timeout: 8000 }).catch(() => null);
    ...
    async function guardedClick(buttonLocator) {
      const name =
        (await buttonLocator.getAttribute('aria-label').catch(() => null)) ||
