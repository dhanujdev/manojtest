...
async function fillField(page, field, value) {
  try {
    ...
    // 2) getByRole fallback (only for known cases)
    ...
    // 3) label:has-text + associated input (fallback)
...
async function extractCompanyAndTitle(page) {
  try {
    const meta = await page.evaluate(() => {
      ...
      let title = null;
      const app = document.querySelector('#application_form');
      const h1InApp = app?.querySelector('h1');
      if (h1InApp) title = text(h1InApp) || null;
      if (!title) {
      ...
      if (c) company = text(c) || null;

      if (!company && title) {
        const h1 = document.querySelector('h1');
        if (h1) {
          const header = h1.closest('header') || h1.parentElement;
          const maybe = header ? text(header).split('·')[0] : '';
          if (maybe && maybe.length < 60 && maybe.toLowerCase() !== title.toLowerCase()) company = maybe;
        }
      }
...
async function extractFields(page) {
  const selector = '#application_form, .application-form, form[action*="greenhouse"]';

  const raw = await page.$$eval(selector, (roots) => {
    function norm(s) {
      return String(s || '').replace(/\s+/g, ' ').trim();
    }
    ...
    const rootEls = roots.length ? roots : [document.documentElement];
    const all = [];
...
export class GreenhouseAdapter {
  ...
  async run(params) {
    ...
    // presentation-only meta extraction
    const meta = await extractCompanyAndTitle(page);
    job.setMeta({ company: meta.company, title: meta.title });

    // Wait for form to render (handles SPAs and slow page loads)
    await page.waitForSelector('#application_form, .application-form, form[action*="greenhouse"], form', { state: 'visible', timeout: 8000 }).catch(() => null);
    ...
    async function anySubmitVisible() {
      ...
      // Also check explicit submit-type buttons and inputs
      const typeSubmit = await page.locator('button[type="submit"], input[type="submit"]').first().isVisible().catch(() => false);
      if (typeSubmit) return true;
      return false;
    }
