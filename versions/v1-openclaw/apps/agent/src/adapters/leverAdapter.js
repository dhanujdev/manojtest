...
async function extractFields(page) {
  const selector = 'form.application-form, form';
  ...
  const raw = await page.$$eval(selector, (forms) => {
    ...
    function labelForInput(el) {
      ...
      // Lever often wraps input in a label-like container
...
export class LeverAdapter {
  ...
  canHandle(url) {
    try {
      ...
      return String(url || '').includes('lever.co') || String(url || '').includes('jobs.lever.co');
    }
  }
  ...
  async run(params) {
    ...
    try {
      ...
      // Lever apply forms are at /company/uuid/apply
      // Job pages are at /company/uuid
      const parts = u.pathname.split('/').filter(Boolean);
      if (parts.length === 2 && !pageUrl.includes('/apply')) {
        // On job listing page — navigate to apply form
        await page.goto(pageUrl + '/apply', { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(800);
      }
    } catch {}
    // Wait for form to actually render (React SPA)
    await page.waitForSelector('form.application-form, form', { state: 'visible', timeout: 8000 }).catch(() => null);

    const hasForm = await page.locator('form.application-form, form').first().isVisible().catch(() => false);
    if (!hasForm) {
